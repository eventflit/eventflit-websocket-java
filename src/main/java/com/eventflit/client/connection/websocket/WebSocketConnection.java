...
public class WebSocketConnection implements InternalConnection, WebSocketListener {
    ...
    private static final String INTERNAL_EVENT_PREFIX = "eventflit:";
    private static final String PING_EVENT_SERIALIZED = "{\"event\": \"eventflit:ping\"}";
    ...
    private void handleInternalEvent(final String event, final String wholeMessage) {
        if (event.equals("eventflit:connection_established")) {
            handleConnectionMessage(wholeMessage);
        }
        else if (event.equals("eventflit:error")) {
    ...
    public void onError(final Exception ex) {
        factory.queueOnEventThread(new Runnable() {
            ...
            public void run() {
                ...
                // call onClose.
                // See:
                // https://github.com/leggetter/eventflit-java-client/issues/8#issuecomment-16128590
                // updateState(ConnectionState.DISCONNECTED);
